...
import 			{MultiFilters, GenericSearchWrap, hostfields, SearchWrapConfig} from './multiFilters.js';
...
export function AggrProcModalCard({rec, parid, aggrMin, endtime, addTabCB, remTabCB, isActiveTabCB, isTabletOrMobile})
{
	...
	return (
		...
		<JSONDescription jsondata={rec} titlestr={`${isaggr ? 'Aggregated' : '' } Process State for '${rec.name}'`} column={2} spanCols={{ cmdline : 3, desc : 3 }} 
			...
			<ButtonModal buttontext={searchtitle} width={'90%'} okText="Cancel"
				contentCB={() => (
					<GenericSearchWrap title={searchtitle} parid={parid}
						inputCategory='process' inputSubsys='extprocstate' maxrecs={50000} filter={filter}
						addTabCB={addTabCB} remTabCB={remTabCB} isActiveTabCB={isActiveTabCB} />
				)} />
